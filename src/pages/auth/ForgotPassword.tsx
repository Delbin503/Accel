...
export default function ForgotPasswordPage() {
  ...
  function submitCode(e: React.FormEvent) {
    ...
    if (joined.length < 6) { setError("Enter all six digits."); return; }
  ...
  return (
    <AuthLayout>
      <div>
        {/* Back link — appears on every step except the final confirmation */}
        {step !== "done" && (
          <button
            onClick={() => {
        ...
        {step === "email" && (
          <>
            ...
            <p className="mt-1 text-[13px] text-muted-foreground">
              Enter the email associated with your Accel account. We'll send you a 6-digit verification code.
            </p>
        ...
        {step === "code" && (
          <>
            <h1 className="text-[26px] font-bold tracking-tight text-foreground">Check your email</h1>
            <p className="mt-1 text-[13px] text-muted-foreground">
              We sent a 6-digit code to <strong className="text-foreground">{email}</strong>. Enter it below to verify your identity.
            </p>
            <form onSubmit={submitCode} className="mt-6 space-y-3">
              <div onPaste={handleCodePaste} className="flex items-center justify-between gap-2">
                {code.map((digit, i) => (
                  <input
                    key={i}
        ...
        {step === "reset" && (
          <>
            <h1 className="text-[26px] font-bold tracking-tight text-foreground">Create new password</h1>
            <p className="mt-1 text-[13px] text-muted-foreground">
              Choose a strong password — at least 8 characters. You'll use this to sign in.
            </p>
        ...
        {step === "done" && (
          <div className="text-center">
            ...
            <h1 className="text-[22px] font-bold tracking-tight text-foreground">Password updated</h1>
            <p className="mt-1 text-[13px] text-muted-foreground">
    ...
    <div>
      ...
      <div className="relative">
        ...
        <Input
          ...
          placeholder={placeholder} className="h-10 px-9 text-[13px]"
        />
