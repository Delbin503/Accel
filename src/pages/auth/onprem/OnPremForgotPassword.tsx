...
import { Mail, Lock, Eye, EyeOff, AlertCircle, KeyRound, ArrowLeft, CheckCircle2, Loader2 } from "lucide-react";
...
export default function OnPremForgotPasswordPage() {
  ...
  function submitReset(e: React.FormEvent) {
    ...
    setTimeout(() => navigate("/on-premise/signin", { replace: true }), 1400);
  }
    ...
    <AuthLayout>
      <div>
        {step !== "done" && (
          <button
            onClick={() => {
              if (step === "email") navigate("/on-premise/signin");
        ...
        {step === "email" && (
          <>
            ...
            <form onSubmit={submitEmail} className="mt-6 space-y-4">
              ...
              <Button type="submit" disabled={sending} className="mt-2 h-10 w-full gap-2 text-base">
                {sending ? (
                  <>
            ...
            <p className="mt-6 text-center text-sm text-muted-foreground">
              Remembered it?{" "}
              <Link to="/on-premise/signin" className="font-semibold text-primary hover:underline">
                Sign in
              </Link>
            </p>
          </>
        ...
        {step === "code" && (
          <>
            ...
            <p className="mt-2 text-base text-muted-foreground">
              We sent a 6-digit code to <strong className="text-foreground">{email}</strong>. Enter it below to verify.
            </p>
        ...
        {step === "reset" && (
          <>
            ...
            <p className="mt-2 text-base text-muted-foreground">Choose a strong password — at least 8 characters.</p>
        ...
        {step === "done" && (
          <div className="text-center">
            ...
            <p className="mt-2 text-base text-muted-foreground">Redirecting you to sign in…</p>
          </div>
        )}
      </div>
    </AuthLayout>
