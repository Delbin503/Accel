...
function StepBadge({ index, current, label }: { index: number; current: number; label: string }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <div className={cn("flex size-7 items-center justify-center rounded-full border text-xs font-bold transition-colors",
      ...
      <div className="hidden flex-col sm:flex">
        <p className={cn("text-sm font-semibold leading-tight", active ? "text-foreground" : "text-muted-foreground")}>
          {label}
        </p>
      </div>
    </div>
              ...
              <div>
                ...
                {areas.length === 0 ? (
                  <div className="flex flex-col items-center justify-center gap-2 rounded-lg border border-dashed border-border py-8 text-muted-foreground">
                    ...
                    <p className="text-sm">No areas yet. Add one above to continue.</p>
                  </div>
                ) : (
                  <div className="space-y-1.5">
                    {areas.map((a) => (
                      <div key={a.id} className="group flex items-center gap-2.5 rounded-lg border border-border bg-background px-3 py-2">
                        ...
                        <span className="flex-1 text-base font-semibold text-foreground">{a.name}</span>
            ...
            <div className="space-y-4">
              ...
              {floorPlanUrl ? (
                <div className="space-y-3">
                  <div className="relative overflow-hidden rounded-xl border border-border bg-neutral-950">
                    ...
                    <button onClick={() => { setFloorPlanUrl(null); setFloorPlanName(""); }}
                      className="absolute right-2 top-2 inline-flex items-center gap-1 rounded-md border border-white/20 bg-black/70 px-2 py-1 text-2xs font-semibold text-white backdrop-blur-sm hover:bg-black/90">
                      <Trash2 className="size-3" />
                      Remove
                    </button>
                  </div>
                  <div>
                    ...
                    <Input value={floorPlanLabel} onChange={(e) => setFloorPlanLabel(e.target.value)}
                      placeholder="e.g. Level 3 — North Wing" className="h-9 text-base" />
                  </div>
                ...
                >
                  ...
                  <p className="text-base font-semibold text-foreground">Click to upload a floor plan</p>
                  <p className="text-xs text-muted-foreground">PNG, JPG or SVG · up to 10 MB</p>
                </button>
              )}
              ...
              {!floorPlanUrl && (
                <button onClick={useSampleFloorPlan}
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-semibold text-foreground">Use Sample Blueprint</p>
                    <p className="text-xs text-muted-foreground">A generic floor plan to use while you wait for the real one.</p>
                  </div>
            ...
            <div className="space-y-4">
              <div className="overflow-hidden rounded-xl border border-border bg-card">
                <div className="border-b border-border px-4 py-3">
                  <p className="text-2xs font-semibold uppercase tracking-widest text-muted-foreground">Site Details</p>
                </div>
                ...
                <div className="border-t border-border px-4 py-3">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {areas.map((a) => (
                      <span key={a.id} className="inline-flex items-center gap-1.5 rounded-md border border-border bg-background px-2 py-0.5 text-xs font-semibold text-foreground">
