...
import { CameraSkeleton, ErrorState, type ForcedState } from "./states";
...
const VIEW_MODES: { key: ViewMode; label: string; icon: React.ElementType; description: string }[] = [
  ...
  // Custom layout mode removed (PRD): only Hero & Wall are offered.
];
...
export default function LiveMonitoringPage({
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* Site context strip — moved to the top, under the toolbar (PRD fix 2) */}
      {selectedSiteInfo && (
      ...
      ) : forced === "empty" || filteredCameras.length === 0 ? (
        <div className="flex flex-col items-center justify-center gap-3 rounded-xl border border-dashed border-border py-20 text-muted-foreground">
          <VideoOff className="size-10 opacity-20" />
          <p className="text-sm">{forced === "empty" ? "No cameras to display." : "No cameras match the current filter."}</p>
        </div>
      ) : viewMode === "hero" ? (
        <HeroView
