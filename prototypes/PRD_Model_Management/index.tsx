...
import RealModelManagement from "./RealModelManagement";
          ...
          <div className="flex min-h-screen w-full bg-background">
            ...
            <div className="flex min-w-0 flex-1 flex-col">
              ...
              <main ref={mainRef} id="main-content" className="flex-1 overflow-auto p-6 focus:outline-none">
                <Routes>
                  <Route path="/models" element={<RealModelManagement forced={forced} onResolveForced={resolve} />} />
                  {/* Real Rule Library so the "Extract from Model" / edit-rule handoff
                      (which navigates to /rules) round-trips back to the model page. */}
...
function NotInPrototype() {
  return (
    <div className="flex flex-col items-center justify-center gap-2 rounded-xl border border-dashed border-border py-24 text-center text-muted-foreground">
      ...
      <p className="text-[12px]">This prototype covers <strong className="text-foreground">Model Management</strong> (plus the Rule Library it hands off to).</p>
    </div>
  );
}
