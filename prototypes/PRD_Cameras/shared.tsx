import { AlertCircle, Cctv, Plus } from "lucide-react";
...
export function ErrorState({ onRetry }: { onRetry: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center gap-3 rounded-xl border border-sev-critical/30 bg-sev-critical/[0.05] py-16 text-muted-foreground">
      ...
      <p className="text-sm text-foreground">Couldn't load cameras.</p>
...
export function EmptyState({ onAdd }: { onAdd: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center gap-3 rounded-xl border border-dashed border-border py-20 text-muted-foreground">
      <Cctv className="size-10 opacity-30" />
      <p className="text-sm font-medium text-foreground">No cameras yet</p>
      <p className="text-[12px]">Add a camera to configure RTSP feeds, NVR linkage, and boundary zones.</p>
